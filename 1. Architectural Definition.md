...
This deliverable must establish a solution that is capable of generating summaries of a group of similar medical trials on a given topic/question, or more simply, produce a systematic review.  The solution will search using publicly available medical databases, and collect trials from open-access and subscribed providers.  Abstractive summaries (or extractive summaries) will be generated from the trials/papers that meet certain standards and relevance.  The output will be converted from text to video.


### 2. Architecture Vision (Business Outcome)
#### 1. Literature Search and meta-analysis
A specific health question or topic will be searched via a health database.  The results returned will be filtered to include only those that meet a set criteria and therefore a certain level of quality.  The chosen papers will form the basis of a systematic review that will prodcuce an overall outcome or opinion. 
#### 2. Abstractive Summarisation
The output of the systematic review will be analysed and an summary produced.  This will be a system generated abstract and will cover; a) participants, b) study, c) results, d) conclusion.
#### 3. Video output
...
The deliverable is primarily an interactive component relying on EBM and publicly available health statistics.  It will be an integral component to search for relevant and qualitative EBM, summarise the EBM, present this data, and to further use the results to question population health statistics.
...
G3: Create an automated systematic review of the primary sources.

G4: Compare the latest EBM findings with current U.K GP prescription data.

G5: Compare the latest EBM findings with current U.K. health spending.

G6: Compare the latest EBM findings with current U.K. health population statistics.

G7: Display the output via social-media and the HillsHealth.org web Front End (WFE).
...
In this work, we will build a systematic review of EBM that is automated and will target key health topics. It will generate an abstractive summary of the collated papers and create a visualisation that will be posted to social media and the current WFE.
...
The scope model below shows which system features are in-scope (inside the boundary) and which ones are out of scope (outside the boundary).  We do not intend to analyse individual trial data i.e. a meta-analysis).  
